...
public static class AtlasHelpContent
{
    ...
    private static readonly Dictionary<string, AtlasHelpPage> Pages = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["scan"] = new(
            ...
            ["Game file count is every recognized file.", "1G1R group count is every unique title group.", "Single-game groups are valid and can be recommended automatically.", "Changing frontend affects the destination structure, not the source files.", "Missing files are not removed just because a later scan targets a different folder."]),
        ["library"] = new(
            "Library",
            "Library lets you browse the games found during the latest scan.",
            ["The wider Systems panel lets you browse one platform at a time without hiding normal console names.", "Manage Systems removes selected systems from PlayBuilder's catalog only.", "System, region, language, format, and duplicate-status filters narrow the list.", "Search finds game titles, filenames, and system names.", "Duplicate groups are groups of two or more releases that appear to represent the same game on the same system. Matching titles on different systems are kept separate.", "Duplicate release indicators show titles with more than one detected release on that system.", "Favorite stars mark catalog games you like; Collections > Favorites builds the playable Favorites collection."],
        ...
        ["collection-rules"] = new(
            ...
            ["1G1R means one selected game file per unique game or title group.", "All Games keeps one best playable copy of every unique game.", "English Only keeps one best English-capable copy of every unique game.", "System search understands common aliases such as SNES, NES, Genesis, Mega Drive, and PSP.", "Select Matching chooses only systems visible after the current system search.", "Language priority decides preferred language before region.", "Region priority breaks ties after language.", "Needs Review means Atlas found unknown or less certain metadata.", "Extra Versions are alternate game files not selected as the winner.", "Checkboxes decide what will be included in Build Preview.", "Favorites search helps select personal favorites; Atlas does not decide personal taste.", "Advanced rule switches are optional expert controls.", "Atlas Profiles are optional advanced presets."],
